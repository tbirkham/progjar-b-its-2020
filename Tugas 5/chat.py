...
class Chat:
    ...
    def list_user(self):
        users = []
        for user in self.sessions.keys():
            users.append(self.sessions[user]['username'])
        return {'status': 'OK', 'user': users}

    def logout_user(self, sessionid):
        username = self.sessions[sessionid]["username"]
        del self.sessions[sessionid]
        return {'status': 'OK', 'messages': 'User {} has succesfully logged out'.format(username)}
